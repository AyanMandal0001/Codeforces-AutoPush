...
void solve() {
  cin >> s ; n = s.length();
   string temp="";
   for(ll i=0;i<n;i++){
      if(i>0&&i<n-1 && s[i]=='v'&& s[i-1]=='o'&&s[i+1]=='o') continue;
      else temp+= s[i]; 
   }
 //  cout << temp << endl;
 // logic : nof of "o"'s X when "o" occur find no of prefix "vvv" and suf "vvv" 
 // vector<ll>v;
   ll cnt = 0;
   ll t = temp.size();
  vector<ll>pre(t,0);
  for(ll i=0;i<t;i++){
    if(temp[i]=='v') {cnt++;}
     ...
     if(cnt>0) pre[i-1] =cnt-1;
       cnt = 0;
    }
  //  cout << cnt << " ";
  }
  if(cnt>0) pre[t-1]=cnt-1;
 // cout << endl;
 //pr(pre);
 
 for(ll i=1;i<t;i++) pre[i]+= pre[i-1];
// pr(pre);
 ll ans = 0,tot=pre[t-1];
 for(ll i=0;i<t;i++){
     if(temp[i]=='o'&& i>1 && i<t-2){
        ll j=i;
        while(temp[j]=='o') j++;
        ll zero = j-i;
     //   cout << zero<<" "<<pre[i-1]<<" "<<tot-pre[i-1]<<endl;
       ans += zero * pre[i-1]*(tot-pre[i-1]);
       i += zero;
    }
 }
cout << ans << endl;
