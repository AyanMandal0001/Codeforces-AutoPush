...
void rec(ll c1,ll c2,ll c3){
  ...
  if(dp[c1][c2][c3]) return;
  dp[c1][c2][c3]=1;
  if(c1>=2) rec(c1-1,c2,c3);
  if(c2>=2) rec(c1,c2-1,c3);
  if(c3>=2) rec(c1,c2,c3-1);
  if(c1>0 && c2>0) rec(c1-1,c2-1,c3+1);
  if(c2>0 && c3>0) rec(c1+1,c2-1,c3-1);
  if(c3>0 && c1>0) rec(c1-1,c2+1,c3-1);
}
void solve() {
   cin>>n>>s;
   // B    G    R
