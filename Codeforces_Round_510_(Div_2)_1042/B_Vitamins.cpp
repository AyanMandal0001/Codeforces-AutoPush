...
ll rec(ll i,ll a,ll b,ll c){
    ...
    string str=v[i].ss;
    ll na=a,nb=b,nc=c;
    if(na==0 && str.find('A')!=string::npos) na=1;
    if(nb==0 && str.find('B')!=string::npos) nb=1;
    if(nc==0 && str.find('C')!=string::npos) nc=1;
    ans=min(ans,cost+rec(i+1,na,nb,nc));
    ans=min(ans,rec(i+1,a,b,c));
...
void solve() {
   ...
   v.resize(n+1);
    for(ll i=0;i<n;i++){
      ll cost;string str;
      cin>>cost>>str;
      v[i]=make_pair(cost,str);
    }
    memset(dp,-1,sizeof(dp));
    ll temp=rec(0,0,0,0);
  if(temp==1e9) cout<<-1<<endl;
  else cout<<temp<<endl;
}
