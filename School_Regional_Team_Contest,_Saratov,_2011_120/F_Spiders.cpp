...
string s;
ll aa[1000100];
void dfs(ll node,ll par,ll curdist,ll &mxdist,ll &farthest,vector<vector<ll>>&adj){
    ...
    for(ll v:adj[node]){
        if(v!=par){
            dfs(v,node,curdist+1ll,mxdist,farthest,adj);
        }
    }
}
ll diameter(ll b,vector<vector<ll>>&adj){
  ll mxdist=-1;
  ll far=1;
  dfs(1,0,0,mxdist,far,adj);
  ll nw=far;
  mxdist=-1;
  dfs(far,0,0,mxdist,nw,adj);
  return mxdist;
}
void solve() {
 cin>>n;
 ...
 for(ll i=0;i<n;i++){
    cin>>b;
    vector<vector<ll>> adj(b+1);
    for(ll edge=0;edge<b-1ll;edge++){
        ll x,y; cin>>x>>y;
        adj[x].pb(y);
        adj[y].pb(x);
    }
