...
ll func(){
    ...
    for(ll len=1;len<n;len++){
        for(ll i=0;i<n;i++){
            if(n%len==0 && (2*len-1)<n){
                cnt=1;
                 ll flag=0;
                 string temp=t.substr(0,len);
                 for(ll i=len;i<n;i++){
                   if(t.substr(i,len)==temp) {cnt++;i+=len-1;}
                   else{ flag=1;break;}
                 }
                 if(flag==0) {
                    mx=max(mx,cnt);
...
void solve() {
   ...
   cin>>t;
   ll frag=func();
   string temp=t.substr(0,n/frag);
  // cout<<temp<<endl;
   int match=0;
   for(ll i=1;i<n;i++){
    if(n-i<=temp.size() && t.substr(i)==temp.substr(0,n-i)){
        match=n-i;
        break;
    }
   }
   string toadd=temp.substr(match);
//   cout<<"Pr"<<toadd<<endl;
    if(toadd.empty()){ toadd=temp; }
    for(int i=1;i<=k-1;i++){
        t +=toadd;
    }  
   cout<<t<<endl;
