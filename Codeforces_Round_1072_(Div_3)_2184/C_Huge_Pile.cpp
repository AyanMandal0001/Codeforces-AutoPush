...
umap<ll,ll>mp;
ll rec(ll level){ 
    if(level< k) return INF;
    if(level==k) return 0;// no more time required
    if(contain(level,mp)) return mp[level];
    ll left = rec(floorf(level,2));
    ll right = rec(ceilf(level,2));
    ll best = min(left,right);
    if(best==INF) {
        mp[level]=INF;
        return INF;
    }
