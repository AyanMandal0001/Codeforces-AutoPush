...
ll rec(ll i,ll st){
    ...
    ans=max(ans,rec(i+1,1));
   if(st==0) {
     ans=max(ans,stu[st][i]+rec(i+2,0));
     ans=max(ans,stu[st][i]+rec(i+1,1));
   }
   else {
      ans=max(ans,stu[st][i]+rec(i+2,1));
     ans=max(ans,stu[st][i]+rec(i+1,0));
   }
    return dp[i][st]=ans;
}
void solve() {
 cin>>n;
 for(ll i=0;i<n;i++) cin>>stu[0][i];
 for(ll i=0;i<n;i++) cin>>stu[1][i];
