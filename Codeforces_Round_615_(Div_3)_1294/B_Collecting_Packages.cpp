...
void solve() {
  ...
  vector<vector<ll>>v;
  for(ll i=0;i<n;i++){
    ll xi,yi;cin>>xi>>yi;  v.pb({xi,yi});
  }
  sort(all(v));
   string ans=""; ll x=0,y=0;
   for(ll i=1;i<=v[0][0];i++) ans+="R";
   for(ll i=1;i<=v[0][1];i++) ans+="U";
   x=v[0][0],y=v[0][1];
  for(ll i=1;i<n;i++){
    ll curx=v[i][0],cury=v[i][1];
   // cout<<endl;
  //  cout<<curx<<" "<<x<<endl;
 //   cout<<cury<<" "<<y<<endl;
    if(curx<x or cury<y) {
    ...
    for(ll j=1;j<=curx-x;j++) ans+="R";
    for(ll k=1;k<=cury-y;k++) ans+="U";
     x=curx; y=cury;
  }
  cout<<"YES"<<endl;
  cout<<ans<<endl;
