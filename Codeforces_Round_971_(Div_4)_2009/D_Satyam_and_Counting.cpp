...
string s;
ll aa[1000100];
bool check(ll x,ll y,map<pair<ll,ll>,ll>&exist){
   if(y==0){
    if(exist.find(make_pair(x-1ll,1ll))!=exist.end() && exist.find(make_pair(x+1ll,1ll))!=exist.end()) return true;
   }
   else{ // y==1
    if(exist.find(make_pair(x-1ll,0ll))!=exist.end() && exist.find(make_pair(x+1ll,0ll))!=exist.end()) return true;
   }
   return false;
}
void solve() {
 ...
 for(auto it=mp.begin();it!=mp.end();it++){
    ...
    if(f==2) {
        stline++;
        ll y1=mp[x][0],y2=mp[x][1];
        if(check(x,y1,exist)) ans++;
        if(check(x,y2,exist)) ans++;
    }
    else{ // f=1
      ll y1=mp[x][0];
       if(check(x,y1,exist)) ans++;
