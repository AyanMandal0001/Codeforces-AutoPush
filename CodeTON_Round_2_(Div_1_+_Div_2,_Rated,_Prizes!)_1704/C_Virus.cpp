...
void solve() { // min no of infected house in the end
  cin >>n >>m;
  vector<ll>v(m);
  ...
  if(n+v[0]-v[m-1]-1>0) seg.pb(n+v[0]-v[m-1]-1);
  sort(seg.begin(),seg.end(),greater<ll>());
 // pr(seg);
 ll ans = 0,day_passed=0;
  for(ll i=0;i<seg.size();i++){
    ll len = seg[i];
    len -= (2*day_passed);
    if(len<=0) break;
   else if(len==1) {ans += 1;day_passed++;}
    else {ans += len-1;day_passed+=2;}
}
  cout << n-ans << endl;
}
