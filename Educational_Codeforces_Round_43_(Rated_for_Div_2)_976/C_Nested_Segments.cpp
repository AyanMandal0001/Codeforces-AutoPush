...
void solve() {
 ...
 for(auto it=pos.begin();it!=pos.end();it++){
    sort(pos[it->first].begin(),pos[it->first].end());
 }
 
 auto it=pos.begin();
 int l=it->first;
 int r=pos[it->first][pos[it->first].size()-1];
 if(pos[it->first].size()>1){
    cout<<mp[{l,pos[it->first][0]}]<<" "<<mp[{l,pos[it->first][1]}]<<endl;
    return;
 }
 it++;
 for(;it!=pos.end();it++){ 
    int nr=pos[it->first][0];
     if(pos[it->first].size()>1){
    cout<<mp[{it->first,pos[it->first][0]}]<<" "<<mp[{it->first,pos[it->first][1]}]<<endl;
    return;
 }
    if(nr<=r){
        cout<<mp[{it->first,nr}]<<" "<<mp[{l,r}]<<endl;
        return;
    }
    l=it->first;
    r=pos[it->first][pos[it->first].size()-1];
 }
 cout<<-1<<" "<<-1<<endl;
}
