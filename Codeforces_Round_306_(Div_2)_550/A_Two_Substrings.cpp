...
string s;
ll dp[100010][2][2];
bool rec(ll i,ll ab,ll ba){
     if(ab>0 && ba>0) return true;
    if(i>=n) return false; 
    if(dp[i][ab][ba]!=-1) return dp[i][ab][ba];
    ll ans=0;
    if(ab==0 && ba==0){
        if(i+1<n && s.substr(i,2)=="AB") ans=ans|rec(i+2,1,0);
       if(i+1<n && s.substr(i,2)=="BA") ans=ans|rec(i+2,0,1);
        ans=ans|rec(i+1,0,0);
    }
    else if(ab==0 && ba>0){
      if(i+1<n && s.substr(i,2)=="AB") ans=ans|rec(i+2,1,1);
        else ans=ans|rec(i+1,0,1);
    }
    else if(ab>0 &&ba==0){
        if(i+1<n && s.substr(i,2)=="BA") ans=ans|rec(i+2,1,1);
          else ans=ans|rec(i+1,1,0);
    }
    return dp[i][ab][ba]=ans;
}
void solve() {
 cin>>s;
 n=s.length();
