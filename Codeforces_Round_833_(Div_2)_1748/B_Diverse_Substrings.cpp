...
void solve() {
 cin>>n>>s;
 // logic : any substring > 100 is invalid
 // as digit is only 0...9
 // 0000..(10times)1111...(10times)and ....9999...(10 times)
 ll ans=0;
 for(ll i=0;i<n;i++){
    vector<ll>f(10,0);
    ll distinct =0;
    ...
    for(ll j=i;j<min(n,i+100ll);j++){
        ll digit=s[j]-'0';
        if(f[digit]==0)distinct++;
        f[digit]++;
        maxf=max(maxf,f[digit]);
        if(maxf<= distinct) ans++;
    }
 }
 cout<< ans<<endl;
}
