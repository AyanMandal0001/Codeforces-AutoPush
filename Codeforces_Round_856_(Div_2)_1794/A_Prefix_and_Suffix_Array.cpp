...
string func(string &t){
    string ans="";
    for(ll i=0;i<t.length()-1;i++) ans += t[i];
    return ans;
}
...
void solve() {
  ...
  if(n==1){
    ...
    cout<<"YES"<<endl; return;
     ...
     if(v[0].substr(1)==func(v[1])) {
        string temp=v[0]+v[1][n-2];
        if(ispalin(temp)) cout<<"YES"<<endl;
        else cout<<"NO"<<endl;
     }
     else if(v[1].substr(1)==func(v[0])){
         string temp=v[1]+v[0][n-2];
          if(ispalin(temp)) cout<<"YES"<<endl;
          else cout<<"NO"<<endl;
     }
  }
