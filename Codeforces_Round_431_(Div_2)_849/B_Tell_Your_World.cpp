...
ll check(ll i1,ll i2){
    ...
    taken[i1]=1;
    taken[i2]=1;
 // slope should be dy/dx 
 for(ll i=1;i<=n;i++){
    ll curx=i,cury=y[i];
   if(i==i1 or i==i2) continue;
   else {
    if(dy*(curx-i1)==(cury-y[i1])*dx) {taken[i]=1;}
   }
 }
 ...
 for(ll i=1;i<=n;i++){
     ll curx=i,cury=y[i];
     // remaining point must satisfy the eqn   y-y1=m*(x-x1) 
    if( !taken[i] and  (dy*(curx-idx))!=((cury-y[idx])*dx)) return 0;
 }
 return 1;
}
void solve() {
 ...
 // logic:  1,2 or 1,3 or 2,3 gives us required slope
 if(check(1,2) or check(1,3) or check(2,3)) cout<<"Yes"<<endl;
 else cout<<"No"<<endl;
}
