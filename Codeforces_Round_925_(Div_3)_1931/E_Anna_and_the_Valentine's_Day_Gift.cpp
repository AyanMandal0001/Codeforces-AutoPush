...
void solve() {
  ...
  for(ll i=0;i<n;i++){
    cin>>aa[i];
    s = to_string(aa[i]); ll t=s.length();
     tot += t;
     ll cnt=0; ll j=t-1;
     while( j>=0 && s[j]=='0'){ cnt++;j--;}
     v.pb(cnt);
  }
  // anna n turns sashs n-1 turns b4 the game ends
  //cout<<tot<<endl;
 ...
 // pr(v);
  ...
  ll temp=tot-minus;
  // as 10^m contains m+1 digits
  if(temp>=m+1) cout<<"Sasha"<<endl;
  else cout<<"Anna"<<endl;
}
