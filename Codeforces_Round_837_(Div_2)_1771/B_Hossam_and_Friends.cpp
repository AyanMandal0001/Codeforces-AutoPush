...
void solve() {
    // cnt of [a,a+1,a+2,…,b] such that any 2 is not friend
  cin>>n>>m;
  vector<ll>enemy(n+1,0);
  // max a who is ememy of b
  for(ll i=0;i<m;i++){
     cin>>a>>b;
     if(a>b) swap(a,b);
     enemy[b]=max(enemy[b],a);
  }
  vector<ll>maxleft(n+1);
  // smallest valid index to start subsegment that ends at i
  maxleft[1]=1; 
  ll ans=1; // for [1] subsegment
  for(ll i=2;i<=n;i++){
    maxleft[i]=max(maxleft[i-1],enemy[i]+1);
    ans += i-maxleft[i]+1;
  }
  cout<<ans<<endl;
}
