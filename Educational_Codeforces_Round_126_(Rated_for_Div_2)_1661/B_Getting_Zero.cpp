...
//   ll op=1e18;
//  if(2ll*val<=32768) op=min(op,1+rec((2ll*val)%32768));
//  else op=min(op,1+rec((val+1)%32768));
//   return dp[val]=op;
// }
void solve() {
  cin>>n;
 ...
 for(ll i=0;i<=32768;i++){
  ...
  dp[0]=0;
  for(ll iter=0;iter<=log2(32768);iter++){
     for(ll i=32768;i>=1;i--){
      dp[i]=min(dp[i],1+dp[(i+1)%32768]);
      dp[i]=min(dp[i],1+dp[(2ll*i)%32768]);
   }
  }
