...
ll mul(ll a, ll b) { return (a % mod1) * (b % mod1) % mod1; }
ll addm(ll a, ll b) { return (a % mod1 + b % mod1) % mod1; }
ll subtm(ll a, ll b) { return (a % mod1 - b % mod1 + mod1) % mod1; }
ll modInv(ll x) { return binpow(x, mod1 - 2, mod1); }
...
ll aa[501];
ll dp[501][501];
// ll rec(ll i,ll left,ll bugs,vector<vector<vector<ll>>>&dp){
//   if(bugs>b) return 0;
//   if(left==0) return 1;
//   if(i>=n) return 0;
//   if(dp[i][left][bugs]!=-1) return dp[i][left][bugs];
//   ll ans=0;
//   ans = (ans+rec(i,left-1,bugs+aa[i],dp))%mod;
//   ans =(ans+rec(i+1,left,bugs,dp))%mod;
//    return dp[i][left][bugs]=ans;
// }
void solve() {
cin>>n>>m>>b>>mod;
for(ll i=0;i<n;i++){
...
//vector<vector<vector<ll>>> dp(n+1,vector<vector<ll>>(m+1,vector<ll>(b+1,-1)));
 dp[0][0]=1;
 for(ll i=0;i<n;i++){
    for(ll left=1;left<=m;left++){
        for(ll bugs=aa[i];bugs<=b;bugs++){
            dp[left][bugs]=(dp[left][bugs]+dp[left-1][bugs-aa[i]])%mod;
        }
    }
 }
 ll tot=0;
 for(ll bugs=0;bugs<=b;bugs++){
    tot = (tot+dp[m][bugs])%mod;
 }
 cout<<tot<<endl;
}
