...
ll rec(ll i,ll prev){
    ...
    ll ans=1e18;
    //if we chose to take rest on current day ,so prev days stays neutral(0)
   ans=min(ans,1+rec(i+1,0));
   // do contest if available
  if((aa[i]==1 || aa[i]==3)&& prev!=1) ans=min(ans,rec(i+1,1));
   //do sport if available
  if((aa[i]==2 || aa[i]==3)&& prev!=2) ans=min(ans,rec(i+1,2));
