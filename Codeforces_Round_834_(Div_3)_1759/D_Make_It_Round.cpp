...
ll zeros(ll number){
    ...
    ll idx=p.length()-1;
    ll cnt=0;
    while(idx>=0 and p[idx]=='0') {idx--;cnt++;}
    return cnt;
}
void solve() {
  cin>>n>>m;
  ll cnt2=0,cnt5=0; ll num=n;
  while(num%2==0) {
      cnt2++;
    num/=2;
  }
  ...
  ll number=1ll;
  if(cnt2>cnt5) {
     while(number*5ll<=m && cnt2>cnt5 ){ cnt5++; number*=5ll;}
  }
  else if(cnt2<cnt5){
      while(number*2ll<=m && cnt2 < cnt5 ){ cnt2++; number*=2ll;}
  }
  while(number*10ll<=m){number *= 10ll;}
    ll temp=(m)/(number);
     cout<<n*temp*number<<endl; 
}
