  ...
  for(ll i=0;i<v.size();i++){
    ...
    //for(ll k=0;k<v.size();k++){
      if(n%(v[i]*v[j])!=0) continue;
        ll pwd = v[i] , pht = v[j] ,pdep = n/(v[i]*v[j]);
     if(w%pwd!=0 || h%pht!=0||d%pdep!=0) continue;
    // if(pwd==1||pht==1||pdep==1) continue;
        cout << pwd-1<<" "<<pht-1<<" "<<pdep-1<<endl;
        return;
//  }
     }
  }
