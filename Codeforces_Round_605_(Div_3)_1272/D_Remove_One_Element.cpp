...
void solve() {
 ...
 for(ll i=1;i<n;i++){
    if(aa[i]>aa[i-1]) pre[i]=pre[i-1]+1;
 }
  for(ll i=n-2;i>=0;i--){
    if(aa[i+1]>aa[i]) suf[i]=suf[i+1]+1;
 }
 
if(n>=2) ans=max(ans,suf[1]);
if(n-2>=0) ans=max(ans,pre[n-2]);
  for(ll i=1;i<n-1;i++){
   ll p=pre[i-1];
   ll s=suf[i+1];
   ll len=0;
   if(aa[i-1]<aa[i+1]) len=max(len,p+s);
   else len=max({len,p,s});
   ans=max(len,ans);
 }
 cout<<ans<<endl;
}
