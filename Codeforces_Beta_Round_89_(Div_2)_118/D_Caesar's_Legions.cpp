...
ll dp[101][101][11][11];
ll MOD=1e8;
ll rec(ll i,ll j,ll cnt1,ll cnt2){
    //pruning
    if(cnt1>k1 or cnt2>k2) return 0;
    // basecase
    if(i==n1&&j==n2) return 1;
    if(dp[i][j][cnt1][cnt2]!=-1) return dp[i][j][cnt1][cnt2];
    ll ans=0;
  if(i<n1)  ans = (ans+rec(i+1,j,cnt1+1,0))%MOD;
  if(j<n2)  ans = (ans+rec(i,j+1,0,cnt2+1))%MOD;
    return dp[i][j][cnt1][cnt2]=ans;
}
void solve() {
 cin>>n1>>n2>>k1>>k2;
